...
void App::drawHostPage()
{
    ...
    ImGui::Text("Ordner im Netzwerk freigeben");
    ImGui::PopStyleColor();
    ...
    if (ImGui::Button("Suchen...", {-1, 0})) cmdBrowseFolder();
    ImGui::PopStyleColor();

    ImGui::PushStyleColor(ImGuiCol_Text, Colors::MUTED);
    ImGui::Text("Freigabe-Name");
    ImGui::PopStyleColor();
    ...
    if (running) {
        ...
        if (ImGui::Button("  Freigabe stoppen  ", {0, 36.f})) cmdStopHost();
        ...
        if (ImGui::Button("  Freigabe starten  ", {0, 36.f})) cmdStartHost();
        ImGui::PopStyleColor(2);
    }
